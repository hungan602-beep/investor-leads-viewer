...
function render() {{
    ...
    document.getElementById('tbody').innerHTML = pageData.map(l => `
        <tr>
            <td>
                <div class="user-cell">
                    <img class="avatar" src="${{l.img}}" onerror="this.src='https://abs.twimg.com/sticky/default_profile_images/default_profile_bigger.png'">
                    <div class="user-info">
    ...
    renderPagination();
    document.getElementById('count').innerText = \`Showing \${{currentData.length > 0 ? start + 1 : 0}}-\${{Math.min(end, currentData.length)}} of \${{currentData.length}} leads\`;
}}
